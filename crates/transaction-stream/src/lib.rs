...
use anyhow::Result;
use futures::prelude::{Stream, stream::FuturesUnordered};
...
impl<'a, St> TransactionStream<'a, St> {
    ...
    pub fn new(provider: &'a DynProvider, stream: St, max_concurrent: usize) -> Self {
        Self {
    ...
    pub(crate) fn push_tx(&mut self, tx: B256) {
        let fut = self
            ...
            .map(move |res| match res {
                Ok(Some(tx_full)) => Ok(tx_full),
                Ok(None) => Err(GetTransactionError::NotFound(tx)),
                Err(err) => Err(GetTransactionError::ProviderError(tx, err)),
            });
    ...
    St: Stream<Item = B256> + Unpin + 'a,
{
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        if !this.stream_done {
            loop {
                match StreamExt::poll_next_unpin(&mut this.stream, cx) {
                    Poll::Ready(Some(tx)) => {
